...
Active Work 是 My-Code-X 中用户当前关注的 Codex 工作现场。

它不是 Codex `Thread` 本身，也不是普通聊天页面。Active Work 面向移动端用户组织一个可进入、可理解、可介入、可恢复的工作现场，让用户在手机上继续使用本地 Codex，而不需要承担 TUI 的交互负担。

Active Work 的设计边界是表达和管理 Codex 工作现场，不重新设计 Codex agent 的智能能力。Codex 负责理解需求、阅读项目、制定计划、执行工具和总结结果；Active Work 负责让这些过程在移动端具备清晰身份、状态、事件呈现、用户介入、移动可靠性和异步连续性。
...
5. 非目标用户不是普通 AI 问答用户。Active Work 不应泛化成通用 chatbot，而应服务用户和本地 Codex 一起完成开发工作。

## Job statements

1. When 我在手机上打开 My-Code-X 时，I want to 快速进入正确的 Codex 工作现场，so I can 继续当前工作而不用重新定位项目、目标或 session。
2. When 页面刷新、切后台、断网或重新打开时，I want to 保持当前 Active Work、草稿和状态不丢失，so I can 信任手机端适合远程协作。
3. When Active Work 正在进行时，I want to 一眼看懂 Codex 是运行、等待、完成、失败、暂停还是断开，so I can 判断自己是否需要介入。
...
8. When 当前 Active Work 需要控制节奏时，I want to 暂停、继续、结束它，so I can 管理长任务。
