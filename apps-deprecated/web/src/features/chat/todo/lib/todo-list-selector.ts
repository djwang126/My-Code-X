import type { SessionNotice } from '../../runtime/public-types';
import type { ActiveChatTodo, ChatTodoStatus, ChatTodoStep } from '../types';

type ParsedTodoNotice = ActiveChatTodo & {
  noticeId: string;
};

function normalizeTodoStatus(value: unknown): ChatTodoStatus {
...
function normalizeTodoSteps(value: unknown): ChatTodoStep[] {
  ...
  return value
    ...
    .filter((entry): entry is ChatTodoStep => Boolean(entry));
}
...
function parseTodoNotice(notice: SessionNotice | undefined, threadId: string): ParsedTodoNotice | null {
  ...
  // Upstream Codex still sends the chat todo list under its `update_plan`
...
export function selectChatTodoState(notices: SessionNotice[] | undefined, threadId: string) {
