...
type RememberWorkspaceThreadInput = {
  path: string;
  threadId: string;
};
...
export function saveWorkspace({ path, label }: WorkspaceDraft): SavedWorkspace {
  const normalizedPath = normalizeWorkspacePath(path);
...
export function rememberWorkspaceThread({ path, threadId }: RememberWorkspaceThreadInput) {
  const normalizedPath = normalizeWorkspacePath(path);
