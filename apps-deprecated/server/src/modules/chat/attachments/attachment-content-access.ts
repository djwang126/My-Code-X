...
function createAttachmentQuery({ slotId, threadId }: { slotId?: string; threadId?: string }) {
...
export function createAttachmentContentUrl({
  attachmentId,
  slotId,
  threadId,
}: {
...
export function normalizeAttachmentAccessContext({ slotId, threadId }: { slotId?: string; threadId?: string } = {}) {
  return {
    slotId: String(slotId || '').trim(),
    threadId: String(threadId || '').trim(),
  };
...
export function assertAttachmentReadable({
  record,
  runtime,
  accessContext,
}: {
  ...
  accessContext: { slotId: string; threadId: string };
}) {
