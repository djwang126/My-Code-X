...
async function hashFile(filePath: any) {
...
export function createAttachmentService({ appDataRoot = path.join(os.homedir(), '.my-code-x'), logger = console, now = () => new Date(), imageAdapter = createSharpImageAdapter(), metadataStore = createAttachmentMetadataRepository({
...
}), resolveSessionRuntime = (_selection?: {
    slotId?: string;
    threadId?: string;
}) => null, processingService = createAttachmentProcessingService({
    ...
    imageAdapter?: any;
    metadataStore?: any;
    resolveSessionRuntime?: (selection?: {
        slotId?: string;
        threadId?: string;
    }) => any;
    processingService?: Partial<ReturnType<typeof createAttachmentProcessingService>>;
    storageService?: Partial<ReturnType<typeof createAttachmentStorageService>>;
} = {}) {
    ...
    async function uploadAttachment({ buffer, contentType, filename, viewerId = 'anonymous', threadId = '' }: any) {
    ...
    async function getAttachmentContent({ attachmentId, slotId, threadId }: any) {
    ...
    async function resolveContent(content: any) {
        const resolved: any[] = [];
    ...
    async function markAttachmentsReferenced({ content, threadId }: any) {
        if (!threadId) {
            return [];
        }
    ...
    return {
        metadataStore,
        setSessionRuntimeResolver(nextResolver: any) {
