...
export function formatStructuredText(value) {
...
function formatTokenUsageBreakdown(label, breakdown = {}) {
    const entries = [
    ...
    return `${label}: ${entries.map(([entryLabel, value]) => `${entryLabel} ${value}`).join(' · ')}`;
}
export function formatTokenUsageText(params = {}) {
...
export function formatThreadStatusText(status) {
    ...
    const activeFlags = Array.isArray(status.activeFlags)
        ? status.activeFlags.filter((flag) => typeof flag === 'string' && flag)
        : [];
...
function formatUserContentItem(item) {
...
function extractHookPromptText(fragments = []) {
    return fragments
        .map((fragment) => {
...
function extractSpecialItemText(item) {
    switch (item?.type) {
        case 'plan':
            return item.text || '';
        case 'reasoning':
        ...
        case 'fileChange':
            return Array.isArray(item.changes) ? item.changes.map((change) => change?.path).filter(Boolean).join(', ') : '';
        case 'mcpToolCall':
...
function createFallbackItemText(itemType) {
    return `[${itemType || 'unknown'}]`;
}
function upsertNormalizedItem(items, nextItem) {
...
export function normalizeCodexThreadItem({ threadId, turnId, turnStatus, item, userMessageOrdinalWithinTurn = 0 }) {
    if (!item?.id) {
        return null;
    }
...
function createNormalizedTimelineItemsFromTurns(threadId, turns = []) {
    const timelineItems = [];
...
export function normalizeResumeThreadResult(result) {
    ...
    const turnExecution = latestTurn
        ? serializeSessionTurnExecution({
            ...
            turnLifecycle: parseCodexTurnLifecycle(normalizeTurnStatus(latestTurn.status), {
...
export function normalizeThreadHistoryEntry(thread) {
    return {
        id: thread?.id || '',
...
export function normalizeThreadListResult(result) {
    const threads = Array.isArray(result?.data) ? result.data : [];
    return threads.map(normalizeThreadHistoryEntry).filter((thread) => thread.id);
}
//# sourceMappingURL=codex-gateway-protocol-normalize.js.map