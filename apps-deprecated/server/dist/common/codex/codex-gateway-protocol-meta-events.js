...
export function createSessionMetaEvent(method, params = {}) {
...
export function createRequestResolvedNotice(params = {}) {
    return {
...
export function createSystemNoticeEvent(method, params = {}) {
        ...
        case 'turn/plan/updated':
            ...
            text = [
                ...
                Array.isArray(params?.plan)
                    ? params.plan.map((step) => `${step?.status || 'pending'}: ${step?.step || ''}`).filter(Boolean).join(' | ')
                    : '',
            ]
                .filter(Boolean)
                .join(' · ');
...
//# sourceMappingURL=codex-gateway-protocol-meta-events.js.map