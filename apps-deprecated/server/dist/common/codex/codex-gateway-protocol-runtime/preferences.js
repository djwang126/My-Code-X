...
export function createDefaultRuntimePreferences() {
    return {
        model: 'gpt-5.4',
...
function readOptionalEnvString(env, key) {
    const value = String(env[key] || '').trim();
    return value || undefined;
}
function readOptionalPositiveIntegerFromEnv(env, key) {
...
export function readRuntimePreferencesEnvDefaults({ env = process.env, } = {}) {
...
export function mergeRuntimePreferencesWithEnvDefaults({ runtimePreferences, env = process.env, }) {
...
export function mapCodexConfigToRuntimePreferences(configReadResponse) {
    ...
    const runtimePreferences = {
        ...
        reasoningEffort: typeof config.modelReasoningEffort === 'string' && config.modelReasoningEffort
            ? config.modelReasoningEffort
            : typeof config.model_reasoning_effort === 'string' && config.model_reasoning_effort
                ? config.model_reasoning_effort
                : fallback.reasoningEffort,
        reasoningSummary: typeof config.modelReasoningSummary === 'string' && config.modelReasoningSummary
        ...
        approvalPolicy: typeof config.approvalPolicy === 'string' && config.approvalPolicy
            ? config.approvalPolicy
            : typeof config.approval_policy === 'string' && config.approval_policy
                ? config.approval_policy
                : fallback.approvalPolicy,
        sandboxMode: typeof config.sandboxMode === 'string' && config.sandboxMode
            ? config.sandboxMode
            : typeof config.sandbox_mode === 'string' && config.sandbox_mode
                ? config.sandbox_mode
                : fallback.sandboxMode,
    };
    const modelContextWindow = readRuntimeNumber(hasOwnKey(config, 'model_context_window') ? config.model_context_window : config.modelContextWindow);
    const modelAutoCompactTokenLimit = readRuntimeNumber(hasOwnKey(config, 'model_auto_compact_token_limit')
