...
function getRawErrorMessage(error, fallbackMessage) {
...
function appendStderrBuffer(buffer, chunk, maxChars) {
...
function getPreferredTransportMessage(stderrBuffer, fallbackMessage) {
...
export async function startCodexJsonlTransport({ command = 'codex', args = ['app-server'], cwd = process.cwd(), env = process.env, requestTimeoutMs = 300000, stderrBufferLimit = 8192, spawnImpl = spawn, } = {}) {
    ...
    let notificationHandler = (_method, _params) => { };
    let serverRequestHandler = (_requestId, _method, _params) => { };
    const pendingRequests = new Map();
    const pendingServerRequestIds = new Map();
    ...
    function setTransportError(error) {
        if (closing)
            return;
        if (transportError)
            return;
    ...
    function setTransportErrorFromFallback(fallbackMessage) {
    ...
    return {
        ...
        setServerRequestHandler(nextHandler) {
            serverRequestHandler = typeof nextHandler === 'function' ? nextHandler : () => { };
        },
        async sendRequest(method, params) {
            return sendRequest(method, params);
        },
        async sendServerRequestResponse(requestId, result) {
...
//# sourceMappingURL=codex-jsonl-transport.js.map