...
import { detectWorkspaceFileContentKind, MAX_WORKSPACE_TEXT_PREVIEW_BYTES, } from './workspace-file-detection.js';
import { createWorkspaceDirectoryEntry, createWorkspaceListedFileEntry, } from './workspace-file-presenters.js';
import { createWorkspaceNotFoundError, isWorkspaceErrorCode, } from './workspace-errors.js';
import { assertTargetWithinWorkspace, realpathWorkspaceTarget, resolveWorkspaceTarget, statWorkspaceTarget, normalizeRelativePath, } from './workspace-paths.js';
function compareWorkspaceListEntries(left, right) {
    if (left.kind !== right.kind) {
        return left.kind === 'directory' ? -1 : 1;
    }
    return left.name.localeCompare(right.name);
}
async function tryResolveRealChildTarget(childTarget) {
    try {
...
async function tryReadChildStats(realChildTarget) {
    try {
...
async function createWorkspaceListEntry({ entry, realWorkspace, relativePath, target, }) {
...
export async function listWorkspaceFiles({ path = '', workspace, }) {
    ...
    const data = await Promise.all(entries.map(entry => createWorkspaceListEntry({
        entry,
        realWorkspace,
        relativePath,
        target,
    })));
    return data.filter((entry) => entry !== null).sort(compareWorkspaceListEntries);
}
//# sourceMappingURL=workspace-list-files.js.map