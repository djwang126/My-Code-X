...
function createDisplayContentUrl({ createContentUrl, accessContext, attachmentId, fallbackUrl = '' }) {
...
export function createAttachmentDisplayContentService({ metadataStore, now, createContentUrl, }) {
    async function toDisplayContentItem(item, accessContext) {
    ...
    async function createDisplayContent(content, accessContext = undefined) {
        if (!Array.isArray(content)) {
            return [];
        }
        return Promise.all(content.map((item) => toDisplayContentItem(item, accessContext)));
    }
    async function hydrateTimelineItems(items, accessContext = undefined) {
...
//# sourceMappingURL=attachment-display-content.js.map