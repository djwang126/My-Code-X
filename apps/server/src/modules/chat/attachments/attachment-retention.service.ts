...
function isExpired(record: any, nowMs: any) {
...
export function createAttachmentRetentionService({ metadataStore, now = () => new Date(), logger, }: any) {
    async function pruneExpiredAttachments() {
        ...
        const keptAttachmentIds: any[] = [];
        const prunedAttachmentIds: any[] = [];
        const missingAttachmentIds: any[] = [];
