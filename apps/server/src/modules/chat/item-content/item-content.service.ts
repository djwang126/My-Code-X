...
export function createItemContentService({ registry }: any) {
    async function getTimelineItemContent({ slotId, threadId, itemId }: any) {
        const runtime = registry.getRuntimeForSelection({ slotId, threadId });
        if (!runtime) {
            throw createHttpError('thread not found', 404);
        }
        const item = runtime.messages.find((message: any) => message.id === itemId);
