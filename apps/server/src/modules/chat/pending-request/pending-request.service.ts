...
export function createPendingRequestService({ codexGateway, now, registry, emitter }: any) {
    async function respondToPendingRequest({ slotId, threadId, requestId, response }: any) {
        const runtime = registry.getRuntimeForSelection({ slotId, threadId });
        if (!runtime) {
            throw createHttpError('thread not found', 404);
        }
        const pendingRequest = runtime.pendingRequests.find((request: any) => request.id === requestId);
        if (!pendingRequest) {
        ...
        try {
            ...
            const currentPendingRequest = currentRuntime?.pendingRequests.find((request: any) => request.id === requestId);
