...
function createDefaultGateway(): CodexGatewayLike {
    return {
        async close() { },
        setNotificationHandler() { },
    };
}
function createLogger(logger: LoggerLike | undefined) {
...
export function createUnconfiguredChatService({ now = () => new Date().toISOString() }: {
    ...
    return {
        async hydrateSession({ viewerId, slotId, workspace = '', threadId = '' }: any) {
...
export function createChatService({ codexGateway: codexGatewayOverrides = createDefaultGateway(), promptOverrideResolver = null, now = () => new Date().toISOString(), logger, attachmentService: injectedAttachmentService = null, attachmentRetentionService: injectedAttachmentRetentionService = null, }: {
    codexGateway?: Partial<CodexGatewayLike>;
    promptOverrideResolver?: PromptOverrideResolver | null;
    now?: () => string;
    logger?: LoggerLike;
    attachmentService?: Partial<ReturnType<typeof createAttachmentService>> | null;
    attachmentRetentionService?: Partial<ReturnType<typeof createAttachmentRetentionService>> | null;
} = {}) {
    const codexGateway: CodexGatewayLike = {
        ...createDefaultGateway(),
        ...codexGatewayOverrides,
    };
    const serviceLogger = createLogger(logger);
    const registry: ChatSessionRegistry = createSessionRegistry();
    const emitter = createChatEventBus();
    const nowAsDate = () => new Date(now());
    const attachmentService = (injectedAttachmentService ?? createAttachmentService({ now: nowAsDate, logger: serviceLogger })) as ReturnType<typeof createAttachmentService>;
    if (!injectedAttachmentService) {
        attachmentService.setSessionRuntimeResolver((selection: any) => registry.getRuntimeForSelection(selection));
    }
    const attachmentRetentionService = (injectedAttachmentRetentionService ??
        (attachmentService?.metadataStore
            ...
            : null)) as ReturnType<typeof createAttachmentRetentionService> | null;
    ...
    function scheduleAttachmentMaintenance(reason: string) {
    ...
    return {
        ...
        async uploadAttachment(input: any) {
        ...
        getSessionState({ slotId, threadId }: any) {
