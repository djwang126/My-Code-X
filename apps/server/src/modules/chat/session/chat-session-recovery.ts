...
function createSessionLogger(logger: any) {
    return {
...
export function createChatSessionRecovery({ attachmentService, codexGateway, logger, now, promptOverrideResolver, registry, }: {
    attachmentService?: {
        hydrateTimelineItems?: (messages: unknown[], context: {
            slotId: string;
            threadId: string;
        }) => Promise<unknown[]>;
    };
    ...
    function rememberAppliedThreadRuntimeOverrides(runtime: any, runtimeSettings: any) {
    ...
    function rememberGatewayAttachment(runtime: any) {
    ...
    function getRuntimeAttachment(runtime: any) {
        if (!runtime?.threadId) {
            return {
                attached: false,
    ...
    function logRuntimeRecovery({ trigger, slotId, threadId, workspace, attachment }: any) {
