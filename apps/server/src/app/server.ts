...
import type { CodexGatewayLike, IdleShutdownConfig, LooseRecord } from '../common/codex/codex-types.js';
import type { PromptOverrideSnapshot } from '../common/codex/codex-types.js';
...
function logCodexEventDebug(stage: string, details: LooseRecord = {}) {
...
async function ensureCodexWorkingDir(codexWorkingDir: string) {
...
export async function startServer({ host = defaultHost, port = defaultPort, authToken = defaultAuthToken, serverInstanceId = defaultServerInstanceId, frontendDistDir = defaultFrontendDistDir, codexCommand = defaultCodexBin, codexArgs = ['app-server'], codexWorkingDir = defaultCodexWorkingDir, customHarnessDir = defaultMyCodeXCustomHarnessDir, codexDynamicTools = defaultCodexDynamicTools, codexEnv, idleShutdownConfig = defaultCodexIdleShutdownConfig, restartScript = defaultRestartScript, }: {
    ...
    customHarnessDir?: string;
    codexDynamicTools?: LooseRecord[];
    codexEnv?: ProcessEnv;
    idleShutdownConfig?: IdleShutdownConfig;
    restartScript?: string;
} = {}) {
    let codexGateway: ReturnType<typeof createCodexGatewayManager> | null = null;
    ...
    try {
        let promptOverrideSnapshot: PromptOverrideSnapshot | null = null;
        ...
        chatService = createChatService({ codexGateway: codexGateway as CodexGatewayLike, promptOverrideResolver });
        codexGateway.setNotificationHandler((event: LooseRecord) => {
