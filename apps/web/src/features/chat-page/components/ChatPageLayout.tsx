...
import { createThreadConversationActions } from '../../thread-actions';
import { useCollapsibleThreadTodo } from '../../thread-todo';
...
export function ChatPageLayout(props: ChatPageProps) {
  ...
  const threadHistory = props.threadHistory ?? [];
  const threadHistoryError = props.threadHistoryError ?? '';
  const threadHistoryLoading = props.threadHistoryLoading ?? false;
  ...
  const { fallbackViewModel, partitionedPendingRequests, activeTodoList, sessionToasts, proposedPlanActionTurnId, showProposedPlanAction } =
  ...
  const { visibleTodo, todoCollapsed, toggleCollapsed } = useCollapsibleThreadTodo({
