...
import { deriveChatPageGuards } from './chat-page-action-guards';
import type { ChatPageGuardInput } from './chat-page-action-guards';
import type {
...
describe('deriveChatPageGuards', () => {
  ...
  it('blocks thread-changing actions and thread mutations outside ready-idle', () => {
    ...
    expect(guards.canOpenThreadHistory).toBe(false);
    expect(guards.canRollback).toBe(false);
    expect(guards.canCompact).toBe(false);
  });

  it('allows thread history and thread mutation actions in ready-idle when a thread exists', () => {
    const guards = deriveChatPageGuards(buildInput());

    expect(guards.canNewThread).toBe(true);
    expect(guards.canOpenThreadHistory).toBe(true);
