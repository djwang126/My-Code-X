...
import { postReviewStart, postThreadCompactStart, postThreadFork, postThreadRollback } from '../thread-actions';
import { fetchWorkspaceFile, fetchWorkspaceFiles, postWorkspaceFileSave } from '../workspace-files';
...
describe('fetchThreadHistory', () => {
  it('requests workspace-scoped thread history', async () => {
  ...
  it('returns the raw backend text when thread history fails', async () => {
    server.use(
    ...
    await expect(fetchThreadHistory({ workspace: 'D:/workspaces/My-Code-X' })).rejects.toThrowError(
      'thread/list failed',
    );
  });
});
...
describe('postAppRestart', () => {
  it('posts a restart request without clearing thread context client-side', async () => {
    ...
    const payload = await postAppRestart({
      viewerId: 'viewer-22',
      slotId: 'tab-5',
      workspace: 'D:/workspaces/My-Code-X',
      threadId: 'thread-22',
