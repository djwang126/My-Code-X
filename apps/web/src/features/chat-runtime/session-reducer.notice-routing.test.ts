...
describe('sessionReducer notice routing', () => {
  it('preserves the active thread id for threadless hardening stream events', () => {
    ...
    const withPendingRequest = sessionReducer(hydrated, {
    ...
    const withNotice = sessionReducer(withPendingRequest, {
    ...
    const resolved = sessionReducer(withNotice, {
  ...
  it('keeps pending request updates data-only while later turn messages arrive', () => {
    ...
    const withLaterMessage = sessionReducer(withPendingRequest, {
    ...
    const withRequestRefresh = sessionReducer(withLaterMessage, {
      type: 'stream/pending-request-updated',
      payload: {
        threadId: 'thread-1',
        request: {
  ...
  it('keeps item-bound pending requests raw even when the item is a special timeline row', () => {
    ...
    const withSpecialItem = sessionReducer(hydrated, {
      type: 'stream/timeline-item-updated',
      payload: {
        threadId: 'thread-1',
        turnId: 'turn-1',
    ...
    const withPendingRequest = sessionReducer(withSpecialItem, {
      type: 'stream/pending-request-updated',
      payload: {
        threadId: 'thread-1',
        request: {
  ...
  it('stores live session meta and system notice events outside the transcript', () => {
    ...
    const withMeta = sessionReducer(hydrated, {
    ...
    const withNotice = sessionReducer(withMeta, {
      type: 'stream/system-notice',
      payload: {
        threadId: 'thread-1',
        notice: {
  ...
  it('clears stale session chrome values when an authoritative snapshot sends empty metadata', () => {
    ...
    const withClearingSnapshot = sessionReducer(withMeta, {
