...
async function openWorkspaceSidebar(user?: ReturnType<typeof userEvent.setup>) {
  if (user) {
...
describe('SessionGate', () => {
  ...
  it('loads workspace thread history and resumes a selected thread from the sidebar', async () => {
    const requestedThreads: string[] = [];
    ...
    await waitFor(() => expect(screen.getByText('Session synced')).toBeInTheDocument());
    await openWorkspaceSidebar(user);
