...
describe('SessionGate transcript error scope', () => {
  ...
  it('keeps thread history load failures out of the transcript log and scopes them to the workspace sidebar', async () => {
    server.use(
    ...
    setSessionIdentity('viewer-thread-history-error', 'tab-thread-history-error');
    ...
    expect(within(transcriptLog).queryByText('Failed to load thread history.')).toBeNull();
    ...
    expect(
      within(screen.getByRole('region', { name: 'thread history' })).getByText('history backend failed'),
    ).toBeInTheDocument();
  });
  ...
  it('renders live session notices in the toast region instead of the transcript log', async () => {
    ...
    const transcriptLog = screen.getByRole('log', { name: 'chat transcript' });
    const toastRegion = await screen.findByRole('region', { name: 'Session toasts' });
